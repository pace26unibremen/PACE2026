#include "Case2BRule.hpp"

solver::Case2BRule::Case2BRule(const std::shared_ptr<graph::Instance>& instance,
...
solver::RuleReturnCode solver::Case2BRule::apply()
{
    if (this->isApplied)
    {
        throw std::invalid_argument("Case2BRule : apply : rule is already applied");
    }
    isApplied = true;
...
void solver::Case2BRule::unapply()
{
    if (not this->isApplied)
    {
        throw std::invalid_argument("Case2BRule : unapply : rule is not applied");
    }
    isApplied = false;
    ...
    for (const auto& [label, node] : f1->LabelToTerminal())
    {
        if (node->sibling != nullptr and f1->TerminalToLabel().contains(node->sibling)) //sibling (c) is leaf
        {
            if (node->parent->parent != nullptr and f1->TerminalToLabel().contains(node->parent->sibling)) //uncle (x) is leaf
            {
                ...
                if (not (a2->parent != nullptr and a2->parent->parent != nullptr and a2->parent->parent->parent != nullptr and a2->parent->parent->parent->parent != nullptr))
                ...
                if (not (f2->TerminalToLabel().contains(a2->sibling) and f2->TerminalToLabel().contains(a2->parent->sibling)))
                ...
                if (doCut)
                {
                    return std::make_shared<Case2BRule>(instance, context, toCutLabelPair);
                }
            }
        }
    }
...
std::string solver::Case2BRule::name() const
{
    return "Case2BRule";
}
